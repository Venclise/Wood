...
import { Minus, ShoppingBag, ShoppingCart, Trash } from "lucide-react";
...
export default function Cart() {
  ...
  return (
    <div>
      <Sheet >
        <SheetTrigger  className="relative bg-transparent hover:bg-gray-100 h-10 w-10 flex items-center justify-center rounded-full text-sm cursor-pointer  ">
          { cart.length ? (
                  <div className="absolute top-0 right-0 text-white bg-[#915745] w-5 h-5 font-mono rounded-full  ">
                     {cart.length } 
                 </div>
          ) : ("")
          }
          ...
          <ShoppingCart className="w-4 h-4 lg:w-5 lg:h-5 " strokeWidth={1} />
...
size="icon-lg"
                           ...
                           className=" w-5 h-5 rounded-full text-red-500 hover:bg-red-100 bg-red-100 cursor-pointer hover:text-red-600 absolute top-0 right-0  "
                         >
...
<Button className=" w-full rounded-xs bg-[#915745] cursor-pointer hover:bg-[#724234]">
      Checkout
</Button>
</SheetClose>
</Link>
