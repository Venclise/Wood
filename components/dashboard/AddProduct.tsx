...
<div className='flex flex-col gap-2'>
 ...
 <ToggleGroup type="single" value={selectedParent} onValueChange={(val) => {
          setSelectedParent(val);
          setSelectedSub(""); 
        }}>
          {categories.map((cat) => (
            <ToggleGroupItem key={cat.title} value={cat.title} className='border rounded-full' onClick={(e) => setProductInfo({...productInfo,category:cat.title})}>
              {cat.title}
            </ToggleGroupItem>
          ))}
        </ToggleGroup>
      ...
      {selectedParent && (
        <>
        <div className="mt-4">
          <h3 className="text-lg font-normal mb-2">Subcategories</h3>
          <ToggleGroup
            type="single"
            value={selectedSub}
          ...
          >
            {categories
              ...
              ?.subCategories.map((sub) => (
                <ToggleGroupItem key={sub.title} value={sub.title} className='border' onClick={(e) => setProductInfo({...productInfo,subcategory:sub.title})}>
                  {sub.title}
                </ToggleGroupItem>
              ))}
          </ToggleGroup>
